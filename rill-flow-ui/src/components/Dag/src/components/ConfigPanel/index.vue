      ...
      switch (cell.attrs.nodeDetail?.task?.category) {
        ...
        default:
          ...
          nodeDetail.value = {
            ...
            status: nodeInfo?.status,
            start_time:
              nodeInfo?.invoke_msg?.invoke_time_infos !== undefined &&
              nodeInfo?.invoke_msg?.invoke_time_infos[0]?.start_time !== undefined
                ? moment(nodeInfo?.invoke_msg?.invoke_time_infos[0]?.start_time).format(
                    'YYYY-MM-DD HH:mm:ss',
                  )
                : null,
            end_time:
              nodeInfo?.invoke_msg?.invoke_time_infos !== undefined &&
              nodeInfo?.invoke_msg?.invoke_time_infos[0]?.end_time !== undefined
                ? moment(nodeInfo?.invoke_msg?.invoke_time_infos[0]?.end_time).format(
                    'YYYY-MM-DD HH:mm:ss',
                  )
                : null,
            context: JSON.stringify(nodeInfo?.invoke_msg?.input),
