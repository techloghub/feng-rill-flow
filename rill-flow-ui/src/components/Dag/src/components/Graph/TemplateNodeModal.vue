...
<script lang="ts" setup>
  ...
  import {
    replaceUItreeData,
    replaceUIWidget,
  } from '@/components/Dag/src/components/NodesBar/methods';
  import {
    convertJSON,
    getJsonData,
    getJsonSchema,
  ...
  const [registerModal, { closeModal }] = useModalInner((cell) => {
    ...
    const data =
      cell.store.data.nodeDetailSchema?.node_type === 'meta'
    ...
    replaceUItreeData(data, getTemplateNodeReferenceByKey(cell.store.data.label));
    ...
    for (const key in schemaParams) {
      console.log('useModalInner schemaInfo jsonSchemaFormData schemaParams', key, schemaParams[key]);
      let inputMappings = cell.store.data.nodeDetailParams.inputMappings
      for (const inputKey in inputMappings) {
        console.log('useModalInner schemaInfo jsonSchemaFormData schemaParams', key, schemaParams[key], inputKey, inputMappings[inputKey]);
        ...
        if (inputTargetParam === key) {
          if(inputMappings[inputKey].source.startsWith("$")) {
            jsonSchemaFormData.value[key] = {attr: "reference", reference: inputMappings[inputKey].source, input: ""}
          } else {
            jsonSchemaFormData.value[key] = {attr: "input", input: inputMappings[inputKey].source, reference: ""}
          }
        }
      }
    }
    ...
    const advancedSettingsParams = cell.store.data.nodeDetailSchema.task_yaml;
  ...
  const handleOk = () => {
    ...
    for (const formKey in jsonSchemaFormData.value) {
      ...
      let formItemValue = jsonSchemaFormData.value[formKey].reference !== ''
